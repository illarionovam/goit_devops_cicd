...
resource "aws_nat_gateway" "nat" {
  allocation_id = aws_eip.nat.id              # Використовуємо ID EIP для NAT Gateway
  subnet_id     = aws_subnet.public[0].id     # Розміщуємо NAT Gateway у першій публічній підмережі

  tags = {
    Name = "${var.vpc_name}-nat-gateway"
  }

  depends_on = [aws_internet_gateway.igw]     # Залежність: спершу має бути Internet Gateway
}
*/